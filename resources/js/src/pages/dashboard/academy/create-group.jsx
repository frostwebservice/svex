...
const CreateNewGroup = (props) => {
  ...
  const validationSchema = Yup.object({
    group_name: Yup.string().max(255).required('This Field is required'),
    brief_group: Yup.string().max(2550).required('This Field is required')
  });
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      if (userinfo.is_admin != '1') {
        if (userinfo.paid == '1' && userinfo.group_cnt >= 5) {
          toast.error(
            'You are trying to create 6th outreach group this month.\n Please upgrade your membership. \n ' +
              userinfo.time +
              'days left by renewal date.'
          );
          return;
        } else if (userinfo.paid == '0') {
          toast.error(
            'Trial user cannot create a outreach group .\n Please upgrade your membership.'
          );
          return;
        }
      }
      ...
      axios
        .post('/api/newgroup', {
          ...
          email: values.email,
          brief_group: values.brief_group
        })
        .then((response) => {
          setIsDone(true);
        });
              ...
              <form noValidate onSubmit={formik.handleSubmit} className="row">
                ...
                <div className="p-1 px-3  ">
                  <RedditTextField
                    ...
                    error={
                      !!(
                        formik.touched.brief_group && formik.errors.brief_group
                      )
                    }
                    onBlur={formik.handleBlur}
                    onChange={formik.handleChange}
